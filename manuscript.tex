...
The understanding of morphological properties of the OS layer and its growth mode is important for the interpretation of the charge transport data presented in the following.
 ...
 Typical evolution of transconductance current for clean graphene and two different coverages is illustrated in Figure \ref{fig:3}, where we show the results of measurements (symbols) obtained on GFETs comprising clean single-layer (brown curve), GFET with PDI8-CN2 deposition of 1 minute and PDI8-CN2 (blue curve) and with deposition of 3 minutes (green curve).
  Only a forward gate sweep (from $V_g<0$ to $V_g>0$) data are shown.
 A minimal hysteresis was observed (See Supplementary information) indicating the density of traps is less than \textbf{XX cm$^{-2}$ - we can calculate this from the dirac shift due to hysteresis} that would originate from the structural and/or chemical defects in the layers.
\TODO{Also shown are the results of fitting to a model function (black dashed curves) explained below}.
...
\begin{figure}[htb]
 ...
 \TODO{Dashed curves represent a model explained in the text}. 
 }
  \label{fig:3}
\end{figure}

We observe transfer characteristics of GFET comprising clean graphene exhibiting charge-neutrality point (CNP) at back-gate voltage of 8 V.
...
\subsection{Transconductance - submonolayer coverage}
\label{sec:first}
...
\subsection{Model}
\label{sec:model}
\TODO{The transconductance models, shown with a dashed line in Figure \ref{fig:4}},
  were devised under the following assumptions.
  ...
  \end{equation}
 $n_o$ represents a carrier concentration induced by impurities\cite{adam-2007}.
 ...
 Effective capacitance can be modelled as a serial connection of the quantum capacitance, given by $C_{\mu} = e^2dn/dE_F$, which results from the Fermi energy ($E_F$) shift in graphene, 
 and the electrostatic geometrical (displacement) capacitance between the graphene layer and the gate electrode, given by $C_{ox}=(\epsilon_0\epsilon)/d_{ox}$\cite{sze-2006,chhikara-2014,das-2008}.
 $C_{\mu}$ is calculated from the dependence of the Fermi energy on the charge density and it was experimentally observed  to be larger than $C_{\mu} \geq 8 \times10^{-7}$ F cm$^{-2}$ \cite{xia_2009}.
 However, $C_{ox}=1.2\times10^{-12}$ F cm$^{-2}$, with dielectric constant of SiO$_{2}$  $\epsilon \approx 4$ and $d_{ox}=$285 nm, which is considerably smaller than $C_{\mu}$.
 Its contribution to the effective capacitance is insignificant.
 Hence, the quantum capacitance in the case of bottom gate contact may therefore be safely neglected\cite{das-2008}. 
 ...
 The total resistance plots were calculated from the transconductance ($R=V_{G}/I_{DS}$) and are presented in Figure \ref{fig:4}.
 The total resistance of the GFET is the resistance of the graphene channel plus the resistance of the contacts.
 The relation of $R$ was obtained from (\ref{eq:2}-\ref{eq:4}):	
 \begin{equation}
	 \label{eq:5}
 ...
 Their corresponding values are summarised in Table \ref{tab:1}.

\begin{figure}[htb]
  \centering
   \includegraphics[width=0.9\textwidth]{./Figures/fig4}  
  \caption{Resistance calculated from the data presented in Figure {\ref{fig:4}} (symbols).
