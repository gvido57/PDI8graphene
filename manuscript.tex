...
\subsection{Transconductance of the complete PDIF8-CN2 layer}
\label{sec:complete}
...
The resistance plots $R=I_{DS}/V_{G}$ from the transconductivity are presented in Figure \ref{fig:4}. The transfer characteristics were modeled under the following assumptions. The gate voltage is a sum of contributions due to the quantum capacitance, given by the position of the Fermi level  ($E_F$) in graphene and the  electrostatic potential difference between the graphene layer and the gate contact, given by $n\cdot e/C_{ox}$ with $C_{ox}$ capacitance of the gate dielectric and $n$  charge carrier concentration\cite{sze-2006}[26].  The dielectric capacitance is $C_{ox}=(\epsilon_0\epsilon)/d_{ox} =1.2\times10^8$ F cm$^{-2}$, with the dielectric constant of SiO$_{2}$  $\epsilon \approx 4$ and $d_{ox}=$300 nm.  Considering the typical charge carrier densities on the order of $10^{13}$ cm$^{-2}$ the potential difference accommodated by the bottom gate contact amounts to 100 V, which is considerably larger than the Fermi level shift in graphene.  Quantum capacitance in the case of bottom gate contact may therefore be safely neglected\cite{das-2008}. The drain current was modeled as:
\begin{equation}
  \label{eq:2}
